...
/**
 * Core не отозвался. Текст один на обе точки, откуда адаптер к нему ходит:
 * команды и цикл опроса (`telegram/polling.js`) — для пользователя это одна и
 * та же неполадка, и объяснять её двумя разными фразами незачем.
 */
export const CORE_UNAVAILABLE_TEXT =
  "Сервис временно недоступен, попробуйте ещё раз через минуту.";

/**
 * Приветствие. Общих примеров запросов здесь нет: они приезжают следом вместе
 * со сводкой по рынку и там привязаны к сегодняшним цифрам — «почему ZEC
 * вырос на 6%» полезнее выдуманного заранее образца.
 *
 * Исключение — RSI, и по двум причинам. Сводка состоит из цен, объёмов и
 * капитализации, индикаторов в ней нет вовсе, поэтому подсказку «так тоже
 * можно» из неё не вычитать. И это единственная возможность с ограничением по
 * монетам: узнать про BTC и ETH из приглашения дешевле, чем из отказа на
 * вопрос про SOL. Текст уходит без разметки (`resetConversation` шлёт его
 * простым сообщением), поэтому пример — в кавычках, а не в кодовом блоке.
 *
 * Если инструмент RSI выключен (`RSI_ENABLED=false` или в образе Core не
 * нашлось Python с TA-Lib), пример обещает то, чего система не сделает:
 * состав реестра инструментов адаптеру не виден, и сверить его неоткуда.
 */
const START_TEXT =
  "Привет! Я — ассистент для криптотрейдеров: собираю рыночные данные с бирж " +
  "и отвечаю на вопросы по трейдингу.\n\n" +
  "Считаю и технические индикаторы — например: «Посчитай RSI по BTC на дневных " +
  "свечах» (RSI пока только для BTC и ETH).\n\n" +
...
const HELP_TEXT =
  ...
  "• Считать индикатор RSI по свечам Binance — пока только для BTC и ETH.\n" +
...
/**
 * Отпускает работу, которая должна пережить обработчик команды.
 *
 * `sendMarketOverview` свои отказы ловит и логирует сам, так что здесь
 * гасится лишь то, чего быть не должно, — иначе это стало бы
 * необработанным отклонением и уронило бы процесс.
 *
 * `collect` передаёт вызывающий код, которому нужно дождаться конца: цикл
 * опроса такого не передаёт (в том и смысл), а тесту без этого пришлось бы
 * угадывать момент готовности таймером.
 *
 * @param {Promise<unknown>} work
 * @param {(work: Promise<unknown>) => void} [collect]
 */
function detach(work, collect) {
  const guarded = work.catch((error) => {
    logError("Непредвиденный отказ фоновой работы команды:", error);
  });
  collect?.(guarded);
}
...
export const commands = [
  ...
  },
  {
    ...
    async handle(ctx) {
      ...
      // Сводку намеренно **не ждём**. Собирается она до двух с половиной
      // минут — модель пишет текст, — а цикл опроса Telegram разбирает
      // апдейты по одному и ждал бы вместе с нами: на это время встал бы
      // весь бот, включая чужие чаты. Тот же принцип, что и с обычными
      // сообщениями: адаптер отдаёт работу и не держит на ней цикл.
      detach(sendMarketOverview(ctx), ctx.background);
    },
  },
  {
    command: "help",
