 ...
 *
 * **Два числа в подписи измеряют разное, и путать их легко.** «На ответ» —
 * вся работа модели по заданию: маршрутизатор, планировщик и отвечающий
 * агент, каждый со своим промптом. Пара в скобках — про окно диалога, а туда
 * попадает лишь отвечающий вызов: промпт маршрутизатора живёт ровно один
 * запрос и к истории отношения не имеет (см. разбор двух величин в
 * `core/src/domain/DialogService.js`).
 *
 * Отсюда «на ответ 2015 токенов» рядом с «(15003/16000 tokens)»: разница
 * между лимитом и остатком — это 997 токенов отвечающего вызова, а остальные
 * 1018 ушли на маршрутизацию и в окно не попали. Вычитать одно из другого
 * нельзя, и подпись «на ответ» стоит именно для этого — без неё первое число
 * читается как расход контекста, и цифры выглядят несходящимися.
 */
...
export function answerFooter(usage) {
  ...
  // Разряды здесь не разделяются: это пара «остаток/лимит», и пробел внутри
  // числа спорил бы с косой чертой за роль разделителя.
  const left = remaining(usage);
  if (left !== undefined) parts.push(`(${left}/${usage.contextLimit} tokens)`);
