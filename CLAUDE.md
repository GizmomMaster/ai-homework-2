...
Следующее направление — `docs/crypto-orchestrator-spec.md`: превращение Core в мультиагентного ассистента для криптотрейдеров (маршрутизатор интентов, планировщик задач, инструменты поверх публичных API бирж). Выполнены фазы 1–2: маршрутизатор интентов (`core/src/agents/`) разводит запрос по четырём веткам; теоретические вопросы отвечает `TheoryAgent` со своим промптом и температурой, задачи пока отклоняются с причиной `task_unsupported` — сбор данных подключается в фазе 4. Агенты строятся на двух каркасах: `JsonAgent` (схема, температура 0) для разбора запроса и `TextAgent` для ответа пользователю. Фаза 3 добавила слой инструментов `core/src/tools/` — пять читающих инструментов поверх публичного API Binance с валидацией параметров, кешем и контрактом отказа; к конвейеру он подключается в фазе 4. Промпт маршрутизатора живёт в `RouterAgent.js` и импортируется скриптом замера `core/scripts/router-eval.mjs` — результаты и вытекающие из них решения в `docs/router-eval-results.md`. План реализации по фазам: https://claude.ai/code/artifact/1411862d-990e-4163-9ec4-7efb4ad88f7a
