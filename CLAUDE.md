...
Следующее направление — `docs/crypto-orchestrator-spec.md`: превращение Core в мультиагентного ассистента для криптотрейдеров (маршрутизатор интентов, планировщик задач, инструменты поверх публичных API бирж). Выполнена фаза 1: маршрутизатор интентов (`core/src/agents/`) разводит запрос по четырём веткам, вне компетенции и уточнения отвечаются без обращения к модели. Теория и задачи пока обе отвечаются как обычный диалог. Промпт маршрутизатора живёт в `RouterAgent.js` и импортируется скриптом замера `core/scripts/router-eval.mjs` — результаты и вытекающие из них решения в `docs/router-eval-results.md`. План реализации по фазам: https://claude.ai/code/artifact/1411862d-990e-4163-9ec4-7efb4ad88f7a
