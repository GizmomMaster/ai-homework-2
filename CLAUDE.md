...
Состояние: фазы 1–4 готовы. Core владеет доменной логикой (`core/src/domain/DialogService.js`), очередью заданий с устойчивой доставкой (`core/src/jobs/`) и SQLite; адаптер сведён к трансляции Telegram ↔ HTTP и не содержит ни БД, ни LLM. Сервисы подтверждают друг друга общим секретом `CORE_AUTH_TOKEN`, запускаются вместе через корневой `docker-compose.yml`. Осталась фаза 5 — миграция существующей БД бота в схему Core.
