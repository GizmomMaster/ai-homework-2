...
#
# Список обязан совпадать с KLINE_INTERVALS из core/src/tools/params.js, и это
# проверяется тестом (core/test/rsi.test.js). Дублируется он намеренно — по
# той же причине, что и список монет: скрипт запускают и руками, и своя
# проверка ему нужна. Но расходиться списки не должны: пока здесь не было
# "1s", планировщик мог выбрать его беспрепятственно, а отказ приходил уже
# после похода за свечами и запуска подпроцесса — и выглядел как поломка
# расчёта, а не как неверный параметр.
INTERVALS = (
    "1s", "1m", "3m", "5m", "15m", "30m",
    "1h", "2h", "4h", "6h", "8h", "12h",
    "1d", "3d", "1w", "1M",
