...
export const TOOL_ERROR = {
  ...
  /**
   * Инструмент работает не со всеми активами, и этот в его список не входит.
   * Отличается от `unknownSymbol`: пара существует и торгуется, ограничение
   * наше, а не биржи, — значит и объяснять его нужно иначе.
   */
  unsupportedAsset: "unsupported_asset",
  /**
   * Не удалось выполнить расчёт у себя: не нашёлся интерпретатор, не
   * установлена библиотека, скрипт упал или завис. К бирже отношения не имеет,
   * поэтому под её коды не маскируется — иначе в отчёте появилось бы «биржа
   * ответила ошибкой» там, где биржа ответила исправно.
   */
