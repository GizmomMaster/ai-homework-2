 ...
 * уточняющих правил.
 *
 * Перечня полей ответа здесь нет намеренно. Он был — списком в фигурных
 * скобках на семь строк, — и пересказывал словами ту же `ROUTER_SCHEMA`,
 * которая уходит в раннер и ограничивает генерацию грамматикой: структурно
 * неверный ответ ей и так невозможен (см. JsonAgent.js). Пересказ стоил ~170
 * токенов на КАЖДЫЙ запрос пользователя — маршрутизатор вызывается на всех
 * ветках, включая отказы, — и был самой дорогой строкой промпта из тех, что
 * ничего не решают. От блока осталась одна строка: два условия, которых
 * JSON Schema выразить не умеет (длина `topicSummary` и то, что
 * `clarificationQuestion` нужен лишь одному интенту).
 *
...
ЗАПОЛНЕНИЕ ПОЛЕЙ: topicSummary — суть темы до 10 слов; clarificationQuestion — только при intent="CLARIFICATION_NEEDED", иначе null.
