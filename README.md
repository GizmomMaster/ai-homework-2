...
| [`dashboard/`](./dashboard) | Дашборд телеметрии: токены, стоимость, timeline заданий |
...
## Телеметрия: дашборд и замер токенов

Core пишет строку на каждый вызов модели и каждый вызов инструмента. Смотреть
это можно двумя способами.

**Дашборд** поднимается вместе с остальным (`docker compose up -d`) и живёт на
<http://localhost:8090> — главная страница показывает токены, условную
стоимость и самые дорогие инструменты, `/jobs/<id>` — timeline одного задания.
Порт публикуется только на `127.0.0.1`: проверки доступа у дашборда нет, а на
странице задания видны тексты запросов.

**Бенчмарк** прогоняет фиксированный набор из 12 сессий (15 заданий) через
весь конвейер на живой модели и снимает все метрики, которые пишет
телеметрия: токены (input, output, reasoning, повторный контекст), стоимость,
задержки, вызовы и отказы — всего, на одно задание, по агентам, по стадиям и
по инструментам. Порядок работы: снять базу, внести правки, перезамерить на
том же наборе. Скрипты лежат внутри образа Core, ставить что-то на хост не
нужно:

```
docker compose exec core node scripts/token-benchmark.mjs --label=база --runs=3 --json=/app/data/base.json
# …правки, затем docker compose up -d --build core…
docker compose exec core node scripts/token-benchmark.mjs --label=после --runs=3 --compare=/app/data/base.json
```

Второй прогон печатает таблицу разницы. Набор запросов зафиксирован и
снабжён отпечатком: если он поменялся, `--compare` об этом предупредит —
сравнивать замеры на разных наборах бессмысленно.

Внутри контейнера `localhost` — это сам контейнер, поэтому в `core/.env`
адрес модели должен быть `http://host.docker.internal:1234` (Ollama —
`:11434`). Если адрес неверный, бенчмарк прервётся на первом задании и скажет
об этом, а не выдаст нули.

Рядом — `router-eval.mjs` (точность маршрутизатора) и `task-eval.mjs`
(точность планировщика), запускаются так же.

### Что дал замер: исходная реализация против оптимизированной

Прогон `--runs=3` на живой модели, 45 заданий на замер, набор запросов один и
тот же. «До» — реализация до сокращения промптов, «после» — текущая.
...
| input всего | 140754 | 118649 | −22105 (−16%) |
| output всего | 13632 | 13310 | −322 (−2%) |
| reasoning | 0 | 0 | 0 |
| повторный контекст (repeated) | 1958 | 1917 | −41 (−2%) |
| repeated, % от input | 1.4% | 1.6% | доля выросла на сжатом знаменателе |
| условная стоимость | $0.6267 | $0.5556 | −$0.0711 (−11%) |
| задержка модели | 6217 мс | 5524 мс | −693 (−11%) |
...
* **Экономия входных токенов не вся принадлежит правкам промптов.** Вывод
  инструментов упал на 41%, а сокращение промптов не может изменить то, что
  возвращает биржа: планировщик на укороченном каталоге построил другие планы,
  и вместе с ними сжался бриф сводящего агента. Офлайн-оценка самих правок
  была ~13%, измерено 16% — разница ровно на этой добавке.
* **Стоимость и задержка упали слабее входных токенов** (−11% против −16%):
  генерация почти не изменилась, и дальше резать статику малорентабельно.
* **Доля повторного контекста выросла с 1.4% до 1.6%** не потому, что повторов
  стало больше — их стало меньше на 2%, — а потому, что сжался знаменатель.
* **Качество не просело:** `router-eval` — 95% строго и 100% мягко
  (единственный промах допустим по спецификации), `task-eval` — 11/11 по
  разбору плана, выполнимости и параметрам, 6/7 по выбору инструмента.

Разбор каждой оптимизации, условия прогонов и открытый вопрос по промаху
планировщика — в
[`docs/token-optimization-results.md`](./docs/token-optimization-results.md).
